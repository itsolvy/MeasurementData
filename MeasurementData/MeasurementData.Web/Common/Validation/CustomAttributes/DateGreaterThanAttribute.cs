...
/// <summary>
/// Specified that a data field value must be greater than specified one.
/// </summary>
[AttributeUsage(
    AttributeTargets.Property | AttributeTargets.Field | AttributeTargets.Parameter,
    AllowMultiple = true
)]
public sealed class DateGreaterThanAttribute : BaseValidationAttribute
{
    ...
    /// </param>
    public DateGreaterThanAttribute(string propertyNameToCompare, bool allowEqualDateTimes = false)
    {
        ...
        var errorMessage =
            ...
            ?? (
                _allowEqualDateTimes
                    ? $"Значение поля \"{checkedFieldInfo.DisplayName}\" должно быть большое или равно значению поля \"{compareFieldInfo.DisplayName}\""
                    : $"Значение поля \"{checkedFieldInfo.DisplayName}\" должно быть больше значения поля \"{compareFieldInfo.DisplayName}\""
            );
